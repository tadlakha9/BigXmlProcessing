package com.soprasteria.springboot.constants;

/**
 * Class for constants in property file
 * @author ajohn
 *
 */
public class PropertyConstants {

	/** constant for folder path */
	public static final String MULTIPROCESSOR_PATH = "multiprocessor.folder.path";
	
	/** constant for output folder path */
	public static final String OUTPUT_PATH = "output.folder.path";
	
	/** constant for error folder path */
	public static final String ERROR_PATH = "error.folder.path";

	/** constant for script path */
	public static final String SCRIPT_PATH = "script.folder.path";

	/** constant for script mail.prpoerties */
	public static final String MAIL_PROPERTIES = "mail.properties";

	/** constant for user name */
	public static final String USER_NAME = "username";

	/** constant for password */
	public static final String PASSWORD = "password";

	/** constant for feedback from */
	public static final String FEEDBACK_FROM = "from";

	/** constant for feedback to */
	public static final String FEEDBACK_TO = "to";
	
	/** constant for search path */
	public static final String SEARCH_PATH = "search.folder.path";
	
	/** constant for root path */
	public static final String ROOT_PATH = "application.root.path";

}
